...
def draw_text(img, msg, pos, font_scale=1, text_color=(0, 255, 0), thickness=2, text_color_bg=None):
    """
    Draws text on the image at the given position.
    If text_color_bg is provided, draws a background rectangle behind the text.
    """
    if text_color_bg is not None:
        (w, h), baseline = cv2.getTextSize(msg, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
        x, y = pos
        cv2.rectangle(img, (x-2, y - h - 2), (x + w + 2, y + 2), text_color_bg, -1)
